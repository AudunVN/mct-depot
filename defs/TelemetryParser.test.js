...
const TestDefinitions = [
    {parser: "JSON", type: "test"},
    {parser: "NA", type: "fc", structPath: "defs/structs/fc_v0.0.3_GeneralT_file.txt"},
    {parser: "NA", type: "fc_startup", structPath: "defs/structs/fc_v0.0.3_StartupT_file.txt"}
];
...
test('can get type for FC telemetry string', () => {
    let parser = new TelemetryParser(TestDefinitions);
    let fcType = parser.getType(fcGeneralSampleString);
...
test('can get definition for FC telemetry type', () => {
    ...
    expect(fcDefinition.type).toEqual("fc");
});
